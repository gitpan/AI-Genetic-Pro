	...
	for my $idx (0..$#$chromosomes){
		...
		if($ga->variable_length){
			...
			if($rand < 0.4 and $range > 2){
				...
				if($rand < 0.6 and $ga->variable_length > 1 and not defined $chromosomes->[$idx]->[0]){
					$chromosomes->[$idx]->[ $min - 1 ] = 1 + random_uniform_integer(1, $#{$_translations->[ $min - 1 ]});
				}else{
					push @{$chromosomes->[$idx]}, 1 + random_uniform_integer(1, $#{$_translations->[scalar @{$chromosomes->[$idx]}]});	
				}
			}else{
				my $id = $min + int rand($range + 1);
				$chromosomes->[$idx]->[$id] = 1 + random_uniform_integer(1, $#{$_translations->[$id]});	
			}
		}else{
